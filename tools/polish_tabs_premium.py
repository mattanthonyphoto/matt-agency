"""
Premium polish for Income, Expenses, Business, Personal, P&L, Mileage, Equipment tabs.
Matches Dashboard/Tax/GST quality with title blocks, KPI cards, section bars,
alternating rows, charts, and proper column widths.
"""
import sys, os, time
...
def alt_rows(reqs, sid, start, end, sc, ec):
    """Apply alternating white/gray to rows (0-indexed, even=white, odd=gray)."""
...
def main():
    ...
    # =========================================================================
    # 1. INCOME TAB — already completed in first run, skip to Expenses
    # =========================================================================
    print("\n[1/7] INCOME TAB — already done, skipping")

    # Jump straight to Expenses
    if True:
        pass  # Income formatting code below is skipped via goto-style

    SKIP_INCOME = True
    if not SKIP_INCOME:
        reqs = []

    # -- Title block: R1(0)=spacer, R2(1)=title 22pt, R3(2)=subtitle, R4(3)=spacer --
    spacer_row(reqs, INC, 0, 15, 16)
    reqs.append(rh(INC, 1, 48))
    reqs.append(rc(INC, 1, 2, 1, 9,
        {"backgroundColor": WHITE, "textFormat": tf(bold=True, sz=22, color=NAVY),
         "verticalAlignment": "MIDDLE"}, F_ALL))
    reqs.append(mg(INC, 1, 2, 1, 9))
    # KPI card in title row: Total Revenue in green
    reqs.append(rc(INC, 1, 2, 10, 13,
    ...
    reqs.append(mg(INC, 1, 2, 10, 13))
    reqs.append(border_box(INC, 1, 2, 10, 13, color=GREEN, style="SOLID", width=1))

    # Subtitle row
    reqs.append(rh(INC, 2, 22))
    reqs.append(rc(INC, 2, 3, 1, 9,
        {"backgroundColor": WHITE, "textFormat": tf(sz=11, color=T4),
         "verticalAlignment": "MIDDLE"}, F_ALL))
    reqs.append(mg(INC, 2, 3, 1, 9))
    # KPI label under card
    reqs.append(rc(INC, 2, 3, 10, 13,
        {"backgroundColor": GREEN_BG, "textFormat": tf(sz=9, color=T3),
         "horizontalAlignment": "CENTER"}, F_BG_TF_AL))
    reqs.append(mg(INC, 2, 3, 10, 13))

    spacer_row(reqs, INC, 3, 15, 12)

    # -- Income Sources section: green header bar row 4(0-idx=4) --
    # We need to insert a section header. Currently R5(idx4) has column headers.
    # Actually, structure: R5=col headers, R6=continuation, R7-R13=data, R14=blank, R15=TOTAL
    # Let's style what exists: make R5(idx4) the green section header
    # Then R6(idx5) = col headers with months split across two rows
    # Actually the current layout: R5 has "Source, YTD, Jan-Jun" and R6 has "Jul-Dec" continuation

    # GREEN section bar for income sources
    section_bar(reqs, INC, 4, 1, 14, GREEN, WHITE, 32)

    # Column headers R5-R6 (idx 4-5) — but R5 is now the section bar.
    # Current: R5(idx4)=col headers, R6(idx5)=continued cols
    # We need to keep R5 as section bar text, R6 as col header
    # Actually let me not merge the section bar since data starts right at row 5
    # Let me just style R5 (idx4) as green section header bar, merged cols 1-14
    # Then style R5/R6 column header rows (idx 4,5)

    # Wait — re-reading the data:
    # R5(idx4): ['', 'Source', 'YTD', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']
    # R6(idx5): ['', '', '', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
    # R7-R13(idx6-12): data rows
    # R14(idx13): blank
    # R15(idx14): TOTAL

    # Style column headers at rows 4-5
    col_header(reqs, INC, 4, 1, 14)
    reqs.append(rh(INC, 4, 30))
    col_header(reqs, INC, 5, 1, 14)
    reqs.append(rh(INC, 5, 26))

    # Data rows R7-R13 (idx 6-12) alternating
    alt_rows(reqs, INC, 6, 13, 1, 14)
    reqs.append(rhs(INC, 6, 13, 26))
    # Right-align numbers
    right_align_cols(reqs, INC, 6, 15, 2, 14)

    # TOTAL row at R15 (idx14)
    reqs.append(rh(INC, 14, 32))
    reqs.append(rc(INC, 14, 15, 1, 14,
        {"backgroundColor": GREEN_BG, "textFormat": tf(bold=True, sz=11, color=GREEN)}, F_BG_TF))
    total_accent(reqs, INC, 14, 1, 14, GREEN, 32)

    # Spacer R16 (idx15)
    spacer_row(reqs, INC, 15, 15, 16)

    # -- Monthly Revenue section: blue header bar at R17 (idx16) --
    section_bar(reqs, INC, 16, 1, 5, BLUE, WHITE, 32)

    # Col headers R18 (idx17)
    col_header(reqs, INC, 17, 1, 5)

    # 12 month rows R19-R30 (idx18-29) alternating
    alt_rows(reqs, INC, 18, 30, 1, 5)
    reqs.append(rhs(INC, 18, 30, 26))
    right_align_cols(reqs, INC, 18, 30, 2, 5)

    # Add SPARKLINE data hint — we'll write a sparkline formula in col D after data
    # For now, add visual "Trend" column header at col 4

    # Blue total row at R31 or after R30 (idx30)
    # Currently R30(idx29) = Dec. Let's put total at idx30
    # We need to write TOTAL row — it may not exist yet

    # Spacer and footer
    spacer_row(reqs, INC, 31, 15, 16)

    # Column widths
    reqs.append(cw(INC, 0, 30))   # A: gutter
    reqs.append(cw(INC, 1, 200))  # B: Source
    reqs.append(cw(INC, 2, 120))  # C: YTD
    for c in range(3, 15):
        reqs.append(cw(INC, c, 100))  # Monthly cols

    reqs.append(freeze(INC, rows=6))

    # Footer
    add_footer(reqs, INC, 33, 1, 9)

    execute_batch(service, reqs, "Income formatting")

    # Write values for Income tab
    val_data = [
        {"range": "'Income'!B2", "values": [["Income"]]},
        {"range": "'Income'!B3", "values": [["All business revenue  ·  2025"]]},
        {"range": "'Income'!K2", "values": [["$102,573"]]},
        {"range": "'Income'!K3", "values": [["Total Revenue"]]},
    ...
    # Add sparkline for monthly revenue in col D (idx 3)
    spark_data = [
        {"range": "'Income'!E17", "values": [["Trend"]]},
        # Sparkline formula in E19
    ]
    service.spreadsheets().values().batchUpdate(
        spreadsheetId=SHEET_ID,
        body={"valueInputOption": "RAW", "data": spark_data}
    ).execute()

    # Write sparkline using USER_ENTERED
    service.spreadsheets().values().update(
        spreadsheetId=SHEET_ID,
        range="'Income'!E19",
        valueInputOption="USER_ENTERED",
        body={"values": [['=SPARKLINE(C19:C30,{"charttype","bar";"color1","#399FDE";"max",15000})']]}
    ).execute()

    print("  Income tab: DONE")
    ...
    # Unmerge existing merges on Expenses first
    unmerge_reqs = [
        um(EXP, 4, 5, 1, 7),
        um(EXP, 23, 24, 1, 7),
        um(EXP, 1, 2, 1, 8),
    ...
    # KPI cards in title row: Business (orange), Personal (purple), Combined (red)
    # Business KPI
    ...
    # -- BUSINESS EXPENSES section: orange bar at R5 (idx4) --
    section_bar(reqs, EXP, 4, 1, 7, ORANGE, WHITE, 32)

    # Column headers implied in data (Category, blank, CRA Line, YTD, Monthly Avg)
    # R6(idx5) is first data row: Equipment (CCA)
    # Data rows R6-R20 (idx 5-19)
    ...
    # -- PERSONAL EXPENSES section: purple bar at R24 (idx23) --
    ...
    # Column widths
    reqs.append(cw(EXP, 0, 30))   # gutter
    reqs.append(cw(EXP, 1, 230))  # Category
    reqs.append(cw(EXP, 2, 60))   # blank
    reqs.append(cw(EXP, 3, 80))   # CRA Line
    reqs.append(cw(EXP, 4, 120))  # YTD
    reqs.append(cw(EXP, 5, 120))  # Monthly Avg
    ...
    # First, we need to insert 4 rows at top for title block
    # Current R1(idx0) = col headers. We need to shift everything down 4 rows.
    insert_reqs = [
        {"insertDimension": {
    ...
    # KPI: YTD Total (orange), Monthly Avg (blue), Top Category (green text)
    ...
    # Sparkline column
    reqs.append(cw(BUS, 16, 120))  # Trend col
    reqs.append(cw(BUS, 0, 230))   # Category
    reqs.append(cw(BUS, 1, 60))    # CRA Line
    reqs.append(cw(BUS, 2, 100))   # YTD
    for c in range(3, 15):
        reqs.append(cw(BUS, c, 78))   # Monthly cols
    reqs.append(cw(BUS, 15, 30))   # blank
    ...
    # KPI: Revenue (blue), Profit (green), Margin (conditional)
    ...
    # Column widths
    reqs.append(cw(PL, 0, 100))   # Month
    reqs.append(cw(PL, 1, 120))   # Revenue
    reqs.append(cw(PL, 2, 120))   # Expenses
    reqs.append(cw(PL, 3, 120))   # Profit
    reqs.append(cw(PL, 4, 80))    # Margin
    reqs.append(cw(PL, 5, 120))   # Cumul Rev
    reqs.append(cw(PL, 6, 120))   # Cumul Profit
    reqs.append(cw(PL, 7, 100))   # vs Target
    reqs.append(cw(PL, 8, 30))    # blank
    reqs.append(cw(PL, 9, 120))   # Trend

    # Gradient on profit column (col D, idx 3): apply conditional formatting
    ...
    # Footer before charts
    # First, clear footer text area
    ...
    # Move footer text (was at rows 19-21, now shifted to 23-25)
    val_data2 = [
    ...
    # Now add charts for P&L
    print("  Adding P&L charts...")

    # Chart 1: Revenue vs Expenses grouped bar chart
    chart_reqs = []
    chart_reqs.append({
        "addChart": {
            "chart": {
                "spec": {
                    ...
                    "titleTextFormat": {
                        "foregroundColorStyle": {"rgbColor": T3},
                        "fontSize": 11, "bold": True, "fontFamily": "Inter"
                    },
                    "basicChart": {
                        "chartType": "COLUMN",
                        ...
                        "axis": [
                            {"position": "BOTTOM_AXIS", "format": {
                                ...
                                "foregroundColorStyle": {"rgbColor": T4}
                            }},
                        ...
                        "domains": [{"domain": {"sourceRange": {
                            "sources": [gr(PL, 5, 17, 0, 1)]
                        }}}],
                        "series": [
                            {"series": {"sourceRange": {
                                "sources": [gr(PL, 5, 17, 1, 2)]
                            }}, "colorStyle": {"rgbColor": BLUE},
                            "targetAxis": "LEFT_AXIS"},
                            {"series": {"sourceRange": {
                                "sources": [gr(PL, 5, 17, 2, 3)]
                ...
                "position": {
                    "overlayPosition": {
                        "anchorCell": {"sheetId": PL, "rowIndex": 20, "columnIndex": 0},
                        "widthPixels": 650, "heightPixels": 350
                    }
                }
            }
        }
    ...
    # Chart 2: Cumulative Profit line chart
    chart_reqs.append({
        "addChart": {
            "chart": {
                "spec": {
                ...
                "position": {
                    "overlayPosition": {
                        "anchorCell": {"sheetId": PL, "rowIndex": 20, "columnIndex": 7},
    ...
    # Title row R1 (idx 0) — already has title text
    ...
    # Style monthly section headers (── JANUARY ── etc) at known rows
    ...
    # Style trip data rows with alternating colors per section
    # We'll do broad alternating for all data areas
    # Between month headers, alternate the data rows
    sections = [
        (6, 11),    # Jan data
        (13, 29),   # Feb data
        (31, 41),   # Mar data
    ...
    # Local driving rows — slightly different (italic)
    # These are the last row before each blank row, containing "Local (various)"
    ...
    # Gas col headers R238 (idx 237)
    col_header(reqs, MIL, 237, 0, 6, 30)

    # Gas receipt rows 238-302 (idx 238-301)
    alt_rows(reqs, MIL, 238, 302, 0, 6)
    reqs.append(rhs(MIL, 238, 302, 22))
    right_align_cols(reqs, MIL, 238, 302, 3, 6)

    # FUEL SUMMARY at R304 (idx 303)
    reqs.append(rh(MIL, 303, 30))
    reqs.append(rc(MIL, 303, 304, 0, 6,
        {"backgroundColor": GREEN_BG, "textFormat": tf(bold=True, sz=11, color=GREEN)}, F_BG_TF))
    total_accent(reqs, MIL, 303, 0, 6, GREEN, 30)

    # Fuel summary data rows 304-307 (idx 304-307)
    alt_rows(reqs, MIL, 304, 308, 0, 8)
    reqs.append(rhs(MIL, 304, 308, 24))
    right_align_cols(reqs, MIL, 304, 308, 3, 6)

    # Column widths
    reqs.append(cw(MIL, 0, 80))    # Date
    reqs.append(cw(MIL, 1, 120))   # From
    reqs.append(cw(MIL, 2, 180))   # To
    reqs.append(cw(MIL, 3, 220))   # Purpose
    reqs.append(cw(MIL, 4, 100))   # KM
    reqs.append(cw(MIL, 5, 80))    # Type
    reqs.append(cw(MIL, 6, 200))   # Evidence
    reqs.append(cw(MIL, 7, 180))   # Notes
    ...
    # Column headers at idx 4 (was 0)
    col_header(reqs, EQP, 4, 0, 12, 32)

    # 12 asset rows idx 5-16 (was 1-12)
    alt_rows(reqs, EQP, 5, 17, 0, 12)
    reqs.append(rhs(EQP, 5, 17, 26))
    right_align_cols(reqs, EQP, 5, 17, 4, 9)

    # TOTALS row — we'll add at idx 18
    # (CCA class reference at idx 11 in the original is now at col K-L, which we keep)

    # Spacer + divider
    spacer_row(reqs, EQP, 17, 14, 16)
    section_divider(reqs, EQP, 17, 0, 10)

    # CCA CLASS REFERENCE section
    # Write it at idx 19
    section_bar(reqs, EQP, 19, 0, 8, NAVY, WHITE, 36)

    # CCA class data rows 20-23
    ...
    # Column widths
    reqs.append(cw(EQP, 0, 230))   # Asset
    reqs.append(cw(EQP, 1, 100))   # Acquired
    reqs.append(cw(EQP, 2, 80))    # CCA Class
    reqs.append(cw(EQP, 3, 60))    # Rate
    reqs.append(cw(EQP, 4, 100))   # Cost
    reqs.append(cw(EQP, 5, 100))   # UCC Start
    reqs.append(cw(EQP, 6, 100))   # CCA Claimed
    reqs.append(cw(EQP, 7, 100))   # UCC End
    reqs.append(cw(EQP, 8, 180))   # Notes
    reqs.append(cw(EQP, 9, 30))    # blank
    reqs.append(cw(EQP, 10, 30))   # blank
    reqs.append(cw(EQP, 11, 220))  # Common Classes
    ...
    # =========================================================================
    # GLOBAL: Set Inter font on all polished tabs
    # =========================================================================
    print("\nApplying Inter font globally on polished tabs...")
    reqs = []
    global_ranges = [
        (INC, 0, 40, 0, 15),
        (EXP, 0, 45, 0, 12),
    ...
    for sid, sr, er, sc, ec in global_ranges:
        reqs.append(rc(sid, sr, er, sc, ec,
    ...
    print("\n" + "="*70)
    print("PREMIUM POLISH COMPLETE")
    print("="*70)
    print("""
  INCOME:
    - Title block: 22pt navy, subtitle, Total Revenue KPI card (green)
    - Column headers with bottom borders
    - 7 income source rows alternating white/gray
    - Green total row with accent border
    - Monthly Revenue section with blue header bar
    - 12 month rows alternating, sparkline bar chart
    - Generous column widths (200px+ for names)

  EXPENSES:
    - Title block with 3 KPI cards: Business (orange), Personal (purple), Combined (red)
    - Orange section header bar for Business Expenses
    - 15 business expense rows alternating
    - Orange total row with accent border
    - Section divider
    - Purple section header bar for Personal Expenses
    - Personal rows alternating, purple total
    - Wide category column (230px)

  BUSINESS:
    - 4-row title block inserted: "Business Expenses" 22pt
    - KPI cards: YTD Total (orange), Monthly Avg (blue), Top Category (green)
    - Column headers with borders, 32px height
    - 16 CRA category rows alternating, 26px each
    - Orange TOTAL row with accent border
    - Navy section bar for Budget vs Actual
    - Budget rows alternating with blue total
    - Sparkline trend column (120px)
    - Category column 230px, monthly columns 78px

  PERSONAL:
    - 4-row title block: "Personal Spending" 22pt
    - KPI cards: YTD Total (purple), Monthly Avg (blue), Daily Avg (cyan)
    - 12 category rows alternating, purple total
    - Navy Insights section header bar
    - Blue Monthly Trend section bar
    - Monthly data rows alternating

  P&L:
    - 4-row title block: "Profit & Loss" 22pt
    - KPI cards: Revenue (blue), Profit (red), Margin (green)
    - 12 month rows alternating
    - Revenue text blue, Expenses text red
    - Gradient color scale on Profit column (red to green)
    - Blue TOTAL row with accent
    - Grouped bar chart: Revenue vs Expenses (650x350)
    - Line chart: Cumulative Profit (550x350)

  MILEAGE:
    - Title: "Mileage Log — 2025" 22pt
    - CRA-compliant subtitle with IT-522R reference
    - Column headers with borders
    - 12 monthly section headers in navy bold
    - Trip data rows alternating per section
    - Local driving rows in italic gray
    - Navy Summary section bar
    - CRA Deduction row highlighted green
    - Orange Gas Receipt Log header bar
    - 64 gas receipt rows alternating
    - Green Fuel Summary row
    - Proper column widths (Date 80px, To 180px, Purpose 220px)

  EQUIPMENT:
    - 4-row title block: "Equipment Register" 22pt
    - KPI cards: Total Assets (blue), CCA This Year (green), Tax Savings (orange)
    - Column headers with borders
    - 12 asset rows alternating
    - Section divider
    - Navy CCA Class Reference section with 4 classes
    - AII Note in italic blue background
    - Wide asset column (230px)
""")
